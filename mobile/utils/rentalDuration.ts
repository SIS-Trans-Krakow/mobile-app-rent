...
/** Strip narrow/no-break spaces often injected by locale formatters (e.g. pl-PL on web). */
function stripInvisible(s: string): string {
  return s.replace(/[\u00a0\u202f\u2007\u2009\u2060\ufeff]/g, '').trim();
}

/**
 * Parse `HH`, `HH:MM`, `HH:MM:SS` or `HH.MM(.SS)` → `HH:MM:SS` (24h) for ISO datetime.
 */
 ...
 *   - 3d 23h => 4 days (4th day started)
 *   - 4d  0h => 4 days (exactly 4 full days)
 *   - 4d  4h30m => 5 days (5th day started)
 * Returns null when timestamps cannot be parsed.
 */
...
/**
 * Days from handover moment until now (same ceil semantics as return vs handover).
 * For active handovers on the detail screen.
 */
export function calculateRentalFullDaysElapsed(
  handoverDate?: string | null,
  handoverTime?: string | null,
  nowMs: number = Date.now(),
): number | null {
  const handoverDt = parseDateTime(handoverDate, handoverTime);
  if (!handoverDt) return null;
  const diffMs = nowMs - handoverDt.getTime();
  if (diffMs <= 0) return 0;
  return Math.ceil(diffMs / MS_PER_DAY);
}
