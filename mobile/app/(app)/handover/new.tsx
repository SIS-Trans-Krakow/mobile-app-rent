...
export default function NewHandoverScreen() {
  ...
  const applySelectedCompany = (company: CompanyItem) => {
    setCompanyId(company.id);
    setSaveCompanyToDb(false);
    setCompanyName(company.name || '');
    setCompanyTaxId(company.tax_id || '');
    setCompanyAddressLine1(company.address_line1 || company.address || '');
    setCompanyAddressLine2(company.address_line2 || '');
    setCompanyPostalCode(company.postal_code || '');
    setCompanyPhone(company.phone || '');
    setCompanyEmail(company.email || '');
    setCompanyContact(company.contact_person || '');
  };

  const {
    searchQuery: companySearchQuery,
    searchResults: companySearchResults,
    searchLoading: companySearchLoading,
    showResults: showCompanyResults,
    selectedCompany,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      formData.append('save_company_to_db', saveCompanyToDb ? '1' : '0');
  ...
  const renderCompanyStep = () => (
    <ScrollContainer contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled" bottomOffset={Spacing.lg}>
      ...
      <CompanyLookup
        label={t('handover.searchCompany')}
        ...
        searchResults={companySearchResults}
        searchingText={t('handover.searching')}
        noResultsText={t('handover.noCompaniesFound')}
        selectedText={t('handover.companyFromDb')}
        onChangeText={onCompanySearchChange}
        onSelectCompany={selectCompany}
        onClearSelection={handleClearCompanySelection}
      />
      ...
      {!selectedCompany ? (
        <>
          <Text style={styles.label}>{t('handover.saveCompanyToDb')}</Text>
          <View style={styles.switchRow}>
            <Switch
              value={saveCompanyToDb}
              onValueChange={setSaveCompanyToDb}
              trackColor={{ false: Colors.gray200, true: Colors.primary }}
              thumbColor={Colors.white}
            />
            <Text style={styles.switchLabel}>
              {saveCompanyToDb ? t('common.yes') : t('common.no')}
            </Text>
          </View>
        </>
      ) : null}
